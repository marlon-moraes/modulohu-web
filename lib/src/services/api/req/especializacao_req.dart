...
class EspecializacaoReq {
  ...
  Future<List<Especializacao>> reqListarEspecializacao(BuildContext context) async {
    Map<String, dynamic> json = {'especializacaoSC': {}};
    http.Response? response = await Requisition(ApiRoutes.listarEspecializacao).req(HttpRequests.POST, json, context, header: _defaultHeaders);
    if (response != null) {
      try {
        final body = jsonDecode(utf8.decode(response.bodyBytes));
        if (body['sucesso']) {
          return (body['especializacaos'] as List).map((e) => Especializacao.fromJson(e)).toList();
        } else {
